...
function testDoubleClickLinks() {
  const opened = [];
  const capture = event => { if (event.target.tagName === 'A') { opened.push(event.target.href); event.preventDefault(); } };
  const check = (condition, message) => { if (!condition) throw new Error(message); };
  const fire = (target, type, options = {}) => {
    const event = new MouseEvent(type, {bubbles: true, cancelable: true, button: 0, detail: type === 'dblclick' ? 2 : 1, ...options});
  ...
  try {
    for (const field of document.querySelectorAll('[data-test-native]')) {
      ...
      field.value = 'Слово https://a.test/ https://b.test/';
      field.dispatchEvent(new Event('input', {bubbles: true}));
      field.focus(); field.scrollLeft = 0; field.scrollTop = 0;
      if (field.selectionStart !== null) field.setSelectionRange(7, 7);
      field.dispatchEvent(new Event('scroll', {bubbles: true}));
      const links = field.closest('.field-case').querySelectorAll('[data-template-external-url]');
      const rect = links[1].getClientRects()[0];
      const point = {clientX: rect.left + 4, clientY: (rect.top + rect.bottom) / 2};
      let count = opened.length;
      fire(field, 'click', point); fire(field, 'click', {...point, detail: 2}); fire(field, 'dblclick', point);
      check(opened.length === count + 1 && opened.at(-1) === 'https://b.test/', field.type + ': double click picked wrong URL');
      count = opened.length;
      const bounds = field.getBoundingClientRect(), style = getComputedStyle(field);
      const word = fire(field, 'dblclick', {...point, clientX: bounds.left + Number.parseFloat(style.paddingLeft) + 3});
      check(opened.length === count && !word.defaultPrevented, field.type + ': ordinary word intercepted');
      fire(field, 'dblclick', {...point, clientX: bounds.right - 2});
      check(opened.length === count, field.type + ': padding opened a link');
      fire(field, 'dblclick', {...point, button: 2});
      check(opened.length === count, field.type + ': right click opened a link');
      field.value = previous; field.dispatchEvent(new Event('input', {bubbles: true})); field.blur();
    }
    const rich = document.getElementById('rich-editor');
    const saved = rich.innerHTML;
    rich.innerHTML = 'Слово <span>https://a.</span><em>test/path</em> и https://b.test/';
    rich.dispatchEvent(new Event('input', {bubbles: true}));
    const range = [...CSS.highlights.get('ais-editable-html-links')].find(item => rich.contains(item.startContainer));
    const rect = range.getClientRects()[0];
    const point = {clientX: rect.left + 3, clientY: (rect.top + rect.bottom) / 2};
    let count = opened.length;
    fire(rich, 'dblclick', point);
    check(opened.length === count + 1 && opened.at(-1) === 'https://a.test/path', 'split rich link double click');
    check(rich.innerHTML.includes('<em>test/path</em>'), 'double click rewrote editor');
    count = opened.length;
    fire(rich, 'click', {...point, ctrlKey: true});
    fire(rich, 'click', {...point, ctrlKey: true, detail: 2});
    fire(rich, 'dblclick', {...point, ctrlKey: true});
    check(opened.length === count + 1, 'Ctrl double click opens more than once');
    rich.innerHTML = window.AISFieldHtmlLinks.renderLinks('https://a.test/');
    count = opened.length;
    fire(rich.querySelector('[data-template-external-url]'), 'dblclick');
    check(opened.length === count + 1 && opened.at(-1) === 'https://a.test/', 'rendered formula link double click');
    rich.innerHTML = saved; rich.dispatchEvent(new Event('input', {bubbles: true}));
    return 'Двойной щелчок: OK (все типы полей, HTML, две ссылки, обычный текст, отступы, Ctrl без повторов)';
  } finally { document.removeEventListener('click', capture); }
}
...
async function runBrowserChecks() {
  ...
  try {
    ...
    lines.push(testDoubleClickLinks());
...
function fixture() {
  return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><title>Живая подсветка ссылок</title>
    ...
    <button id="run-checks" type="button">Повторить проверки</button><h2>Журнал тестовых переходов (без открытия сайтов)</h2><pre id="open-results"></pre></main><script src="/field-html-links.js"></script>
    <script>${testDoubleClickLinks.toString()}
    document.addEventListener('click', event => { if (event.target.tagName === 'A') { event.preventDefault(); document.getElementById('open-results').textContent += event.target.href + '\\n'; } });
    const run = ${runBrowserChecks.toString()}; document.getElementById('run-checks').addEventListener('click', run); window.addEventListener('load', run);</script></body></html>`;
}

if (process.argv.includes("--serve")) {
  const server = http.createServer((request, response) => {
    if (request.url === '/shutdown' && request.method === 'POST') {response.end('Stopped'); server.close(); clearTimeout(expiry); return;}
  ...
  new (require("node:vm").Script)(`${testDoubleClickLinks.toString()} const run = ${runBrowserChecks.toString()};`);
