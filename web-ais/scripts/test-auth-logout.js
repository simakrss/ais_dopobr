...
assert.match(indexSource, /new XMLHttpRequest\(\)/u);
assert.match(indexSource, /auth-bootstrap\.js\?v=\$\{encodeURIComponent\(build\)\}&transport=xhr/u);
assert.match(indexSource, /script\.textContent = `\$\{request\.responseText\}/u);
