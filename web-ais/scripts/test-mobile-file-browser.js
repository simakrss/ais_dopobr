...
(async()=>{
  const {PDFDocument,rgb}=require("../vendor/pdf-lib.min.js");const doc=await PDFDocument.create();
  for(let i=0;i<2;i++){const page=doc.addPage([595,842]);page.drawRectangle({x:30,y:30,width:535,height:782,color:rgb(i?0.1:0.75,0.25,i?0.75:0.1)});}
  const pdf=Buffer.from(await doc.save());
  const browser=await chromium.launch({headless:true,...(process.env.PLAYWRIGHT_CHANNEL?{channel:process.env.PLAYWRIGHT_CHANNEL}:{})});
  try {
    const page=await browser.newPage({viewport:{width:360,height:640},isMobile:true,hasTouch:true,deviceScaleFactor:3}),errors=[];
    await page.addInitScript(()=>Object.defineProperty(navigator,'pdfViewerEnabled',{get:()=>false}));
    page.on("pageerror",e=>errors.push(e.message));
    await page.route("**/*",async route=>{
      const url=new URL(route.request().url());
      if(['pdf-preview.js','pdfjs-core.js','pdfjs-worker.js'].includes(url.pathname.slice(1)))return route.fulfill({contentType:'text/javascript',body:fs.readFileSync(path.join(root,url.pathname.slice(1)))});
      if(url.pathname==='/test.pdf')return route.fulfill({contentType:'application/pdf',body:pdf});
      if(url.pathname==='/denied.pdf')return route.fulfill({status:403,contentType:'application/json',body:JSON.stringify({error:'Нет доступа к тестовому PDF'})});
      if(url.pathname==='/broken.pdf')return route.fulfill({contentType:'application/pdf',body:'not a PDF'});
      if(url.pathname==='/slow.pdf'){await new Promise(resolve=>setTimeout(resolve,400));return route.fulfill({contentType:'application/pdf',body:pdf}).catch(()=>{});}
    ...
    await page.addScriptTag({content:`
      ...
      const entries=[{name:'Фотография_документа_с_длинным_названием.jpg',path:'image',size:12345,previewKind:'image'},
        ...
        ...['denied','broken','slow'].map(path=>({name:path+'.pdf',path,size:123,previewKind:'pdf'}))];
      ...
      const getStudentWebDavDocumentFileUrl=(folder,file)=>file==='image'?'/test.svg':['denied','broken','slow'].includes(file)?'/'+file+'.pdf':'/test.pdf';
    ...
    for(const kind of ['students','contracts']){
      for(const size of [{width:320,height:568},{width:390,height:844},{width:844,height:390}]){
        ...
        assert.equal(await page.locator('[data-student-webdav-preview-pdf]').evaluate(el=>el.tagName),'DIV');
        assert.equal(await page.locator('[data-student-webdav-preview] iframe').count(),0);
        await fits('[data-pdf-scroll]',80);
        assert.equal(await page.locator('[data-pdf-page]').textContent(),'1 / 2');
        assert.ok(await page.locator('[data-pdf-canvas]').evaluate(canvas=>{const c=canvas.getContext('2d').getImageData(canvas.width/2,canvas.height/2,1,1).data;return c[3]===255&&c[0]>c[2]*2;}),'Actual coloured PDF pixels, not just a loaded iframe');
        assert.ok(await page.locator('[data-pdf-canvas]').evaluate(c=>c.width*c.height<=4000000));
        await page.click('[data-pdf-next]');await page.waitForFunction(()=>document.querySelector('[data-pdf-page]').textContent==='2 / 2');
        assert.ok(await page.locator('[data-pdf-canvas]').evaluate(canvas=>{const c=canvas.getContext('2d').getImageData(canvas.width/2,canvas.height/2,1,1).data;return c[2]>c[0]*2;}));
        await page.click(action('preview-rotate-right'));await page.waitForFunction(()=>{const c=document.querySelector('[data-pdf-canvas]');return c.width>c.height;});
        await page.click(action('preview-rotate-left'));await page.waitForFunction(()=>{const c=document.querySelector('[data-pdf-canvas]');return c.height>c.width;});
        const initialWidth=await page.locator('[data-pdf-canvas]').evaluate(c=>parseFloat(c.style.width));
        await page.click(action('preview-zoom-in'));await page.waitForFunction(w=>parseFloat(document.querySelector('[data-pdf-canvas]').style.width)>w,initialWidth);
        await page.click(action('preview-fit'));await page.waitForFunction(w=>Math.abs(parseFloat(document.querySelector('[data-pdf-canvas]').style.width)-w)<1,initialWidth);
        if(kind==='students'&&size.width===390&&process.env.PDF_SCREENSHOT)await page.screenshot({path:process.env.PDF_SCREENSHOT});
    ...
    await open('students');
    await page.click('[data-webdav-browser-entry="denied"]');await page.waitForSelector('.student-webdav-browser-pdf-stage.is-load-error');
    assert.match(await page.locator('[data-student-webdav-media-loading]').textContent(),/Нет доступа/);
    await page.click(action('preview-expand'));await page.click('[data-webdav-browser-entry="broken"]');
    await page.waitForSelector('.student-webdav-browser-pdf-stage.is-load-error');
    assert.notEqual(await page.locator('[data-student-webdav-preview-pdf]').getAttribute('data-preview-ready'),'true');
    await page.click(action('preview-expand'));await page.click('[data-webdav-browser-entry="slow"]');
    await page.click(action('preview-expand'));await page.click('[data-webdav-browser-entry="image"]');
    assert.equal(await page.evaluate(()=>requests.filter(r=>r.url==='/slow.pdf').every(r=>r.signal.aborted)),true);
    await page.waitForFunction(()=>document.querySelector('[data-student-webdav-preview-image]')?.naturalWidth>0);
    await page.click(action('preview-expand'));await page.click('[data-webdav-browser-entry="slow"]');
    await page.click('[data-action="close-student-webdav-browser"]');
    assert.equal(await page.evaluate(()=>requests.filter(r=>r.url==='/slow.pdf').every(r=>r.signal.aborted)),true);
    assert.deepEqual(errors,[]);
    console.log('PASS: both card browsers at 320/390/844px, real PDF canvas without native viewer, page navigation/zoom/rotation, 4MP limit, 403/corrupt PDF, abort on switching/closing, image/text/downloads, long warnings and desktop split view');
