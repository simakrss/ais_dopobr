...
async function uiTests(){
 ...
 vm.createContext(c);vm.runInContext(["escapeHtml","escapeAttr","getFilteredSocialPrograms","renderSocialMaterials","loadSocialMaterials","copySocialMessage"].map(name=>extract(name)).join("\n"),c);
...
backendTests();Promise.all([routeTests(),uiTests()]).then(()=>{if(process.argv.includes("--serve"))serveFixture();}).catch(error=>{console.error(error);process.exitCode=1;});
