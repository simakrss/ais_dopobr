...
(() => {
  ...
  function mount(host, blob, {fileName = "Документ.pdf", baseUrl = document.baseURI} = {}) {
    ...
    host.innerHTML = `<div class="pdf-preview-toolbar" role="toolbar" aria-label="Просмотр PDF">
      ...
      <button type="button" data-pdf-minus aria-label="Уменьшить">−</button>
      <button type="button" data-pdf-plus aria-label="Увеличить">+</button>
      <a data-pdf-open target="_blank" rel="noopener">Открыть PDF ↗</a>
      <a data-pdf-download>Скачать</a>
    ...
    controls();
    function error(message) { if (!closed) { status.hidden = false; status.textContent = message; } }
    async function render() {
      ...
      do {
        ...
        try {
          ...
          if (!closed && current === sequence) { label.textContent = `${number} / ${pdf.numPages}`; status.hidden = true; }
          page.cleanup();
