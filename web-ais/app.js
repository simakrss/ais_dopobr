...
Телеграмм - https://t.me/zifra_plus`,
  ...
  const documentTemplatePublicSourceMigrations = Object.freeze({
    "https://disk.yandex.ru/i/_3SnxIXyPuVaZQ": "Документы/Заявление+договор_ДПП (повыш. квалиф. и профпереп.).docx",
    "https://disk.yandex.ru/i/FR1VbW_1EtRHAQ": "Документы/Заявление_ДПП (повыш. квалиф. и профпереп.).docx",
    "https://disk.yandex.ru/i/eyxERaRJXPCC4A": "Документы/Диплом о переподготовке_v1.docx",
    "https://disk.yandex.ru/i/lAQO0kGbtWTjSA": "Документы/Удостоверение о повышении квалификации_v1.docx",
    "https://disk.yandex.ru/i/VSPdwaaaFKUK9g": "Документы/Сертификат ПРО.docx",
    "https://disk.yandex.ru/i/2SP7KB2PpwykbQ": "Документы/Справка подтверждающая обучение.docx",
    "https://disk.yandex.ru/i/a5MJDB7nQQYptg": "Документы/Конверт C5 почтовое отправление документа об образовании.docx",
    "https://disk.yandex.ru/i/cqN9Hcs4j_dgUw": "Документы/Приказ о зачислении (с подписью).docx",
    "https://disk.yandex.ru/i/6TdhHJVX2cZl3A": "Документы/Приказ об отчислении (с подписью).docx"
  });
  const defaultDocumentTemplateWebDavSources = Object.freeze({
    ...
    enrollmentOrder: documentTemplatePublicSourceMigrations["https://disk.yandex.ru/i/cqN9Hcs4j_dgUw"],
    expulsionOrder: documentTemplatePublicSourceMigrations["https://disk.yandex.ru/i/6TdhHJVX2cZl3A"]
  });
  const contractTemplateSettingDefaults = [
    {
      key: "templateUrl",
      label: "WebDAV-путь или ссылка на шаблон",
      value: defaultDocumentTemplateWebDavSources.contract,
      type: "text",
      required: false
    },
    {
  ...
  function createLegalEntityApplicationDocumentTemplate() {
    return {
      ...
      title: "Заявление",
      templateUrl: defaultDocumentTemplateWebDavSources.legalApplication,
      templatePath: "",
  ...
  const educationDocumentTemplateDefinitions = [
    {
      ...
      templateUrl: defaultDocumentTemplateWebDavSources.diploma,
    ...
    },
    {
      ...
      templateUrl: defaultDocumentTemplateWebDavSources.qualificationCertificate,
  ...
  const studyCertificateDocumentTemplateDefinition = {
    ...
    templateUrl: defaultDocumentTemplateWebDavSources.studyCertificate,
    templatePath: "",
  ...
  const postalEnvelopeDocumentTemplateDefinition = {
    id: postalEnvelopeDocumentTemplateId,
    title: "Почтовый конверт",
    templateUrl: defaultDocumentTemplateWebDavSources.postalEnvelope,
    templatePath: "",
    ...
    formulas: {
      "ФИО": "=[ФИО]",
      "Адрес": `=СЖПРОБЕЛЫ(ПСТР([Адрес места жительства];ПОИСК(",";[Адрес места жительства])+1;ДЛСТР([Адрес места жительства])))`,
      "Телефон": "=[Обуч тел#]",
  ...
  const orderDocumentTemplateDefinitions = [
    ...
    },
    {
      ...
      formulas: {
        ...
        "N2": "=ЕСЛИ(\"#СписокБезВыдачи|5#\"<>\"\";ЕСЛИ(\"#СписокСвыдачей|5#\"<>\"\";2;1);\"\")",
  ...
  function createEducationDocumentTemplate(definition, index = 0) {
    const fields = definition.markers.map((name, fieldIndex) => ({
      ...
      position: definition.fieldPositions?.[name] || fieldIndex + 1,
      name,
  ...
  function getDefaultDocumentTemplates() {
    return [
      ...
      ...orderDocumentTemplateDefinitions.map((definition, index) => (
        createEducationDocumentTemplate(definition, educationDocumentTemplateDefinitions.length + index + 2)
      ))
    ];
  }
  ...
  const configs = {
    ...
    trainingPlans: {
      ...
      fields: [
        ...
        field("attestation", "Аттестация", "select", false, "trainingPlanAttestationTypes"),
  ...
  const expenseFields = Array.from({ length: 6 }, (_, index) => [
    ...
    field(`expense${index + 1}InventoryId`, `Запас ${index + 1}`),
    field(`expense${index + 1}InventoryLink`, `Связь с запасами ${index + 1}`)
  ]).flat();
  ...
  function ensureDataShape(data) {
    ...
    data.meta.studentDatabaseWebDavPath = String(
      data.meta.studentDatabaseWebDavPath || DEFAULT_STUDENT_DATABASE_WEBDAV_PATH
    ).trim();
    delete data.meta.studentDatabaseUrl;
    delete data.meta.studentPhotoBasePath;
    delete data.meta.systemDocumentsPublicUrl;
  ...
  function normalizeStudentGender(value) {
    ...
    const normalized = text.toLocaleLowerCase("ru-RU");
    if (normalized === "ж" || normalized.startsWith("жен")) return "Женский";
    if (normalized === "м" || normalized.startsWith("муж")) return "Мужской";
    return text;
  }
  ...
  function normalizeDocumentTemplateSource(value) {
    const source = String(value || "").trim();
    if (!source) return "";
    const migratedSource = documentTemplatePublicSourceMigrations[source] || source;
    if (/^https?:\/\//i.test(migratedSource)) return normalizeExternalUrl(migratedSource);
    if (/^(?:disk\.yandex\.ru|yadi\.sk|webdav\.yandex\.ru)\//i.test(migratedSource)) {
      return normalizeExternalUrl(`https://${migratedSource}`);
    }
    if (/^webdav:\/\//i.test(migratedSource)) return migratedSource;
    const parts = migratedSource
      .replace(/\\/g, "/")
      .replace(/^\/+|\/+$/g, "")
      .split("/")
      .map((part) => part.trim())
    ...
    return contractTemplateSettingDefaults.map((setting) => {
      ...
      return {
        ...
        value: setting.key === "templateUrl"
          ? normalizeDocumentTemplateSource(savedSetting?.value ?? setting.value)
          : String(savedSetting?.value ?? setting.value)
      };
    });
  }
  ...
  function attachDirectExpensesToStudentRecords(students = [], directExpenses = []) {
    ...
    const linkedIdsByStudent = new Map();
    students.forEach((student) => {
      const linked = Array.isArray(student.directExpenses)
        ? student.directExpenses.filter((expense) => expense && typeof expense === "object")
        : [];
      student.directExpenses = linked;
      linkedIdsByStudent.set(
        student,
        new Set(linked.map((expense, index) => directExpenseIdentity(expense, index)))
      );
    ...
    directExpenses.forEach((expense, index) => {
      ...
      const linkedIds = linkedIdsByStudent.get(student);
      if (!linkedIds.has(identity)) {
        student.directExpenses.push(expense);
        linkedIds.add(identity);
      }
  ...
  function normalizeTrainingPlanRecord(item, index = 0) {
    ...
    return {
      ...
      programId: String(value("programId", "programID", "programUid", "ID программы")).trim(),
      code: String(value("code", "Код", "№", "N") || item.code || index + 1).trim(),
      ...
      content: String(value("content", "Содержание", "Темы")).trim()
    };
  }

  function getTrainingPlanSeedRows() {
    return Array.isArray(window.AIS_SEED?.collections?.trainingPlans)
      ? window.AIS_SEED.collections.trainingPlans.map((item, index) => normalizeTrainingPlanRecord(item, index))
      : [];
  }

  function trainingPlanImportKey(item) {
    return [
      item?.code,
      item?.programName,
      item?.discipline,
      ...
      item?.practiceHours,
      item?.attestation
    ].map((value) => normalizeProgramName(value)).join("|");
  }

  function mergeTrainingPlanSeed(currentRows = [], data = {}) {
    const seedRows = getTrainingPlanSeedRows();
    if (!seedRows.length || currentRows.length >= seedRows.length) return currentRows;
    data.meta = data.meta && typeof data.meta === "object" ? data.meta : {};
    data.meta.trainingPlanSeedVersion = `${window.AIS_SEED?.meta?.sourceWorkbook || ""}:${window.AIS_SEED?.meta?.extractedAt || ""}:${seedRows.length}`;
    const seedKeys = new Set(seedRows.map(trainingPlanImportKey));
    const customRows = currentRows.filter((item) => (
      String(item?.programId || "").trim() && !seedKeys.has(trainingPlanImportKey(item))
    ));
    return [...seedRows, ...customRows];
  }

  function linkTrainingPlanRecordsToPrograms(rows = [], programs = []) {
    return rows.map((item) => {
      const row = normalizeTrainingPlanRecord(item);
      if (String(row.programId || "").trim()) return row;
      ...
      <aside class="sidebar">
        ...
        <nav class="nav-list">
          ${orderedNavItems.map((item) => `
            <button class="nav-item ${state.view === item.id ? "active" : ""}" data-view="${item.id}" data-nav-item="${item.id}" type="button" draggable="true">
  ...
  function renderCollection(config) {
    ...
    const importedViewIds = state.view === "students"
      ? (state.studentImportedViewIds || [])
      : [];
    const importedViewCount = state.view === "students"
      ? importedViewIds.filter((id) => (
      ...
      <section class="panel">
        ...
        ${importedViewIds.length ? `
          <div class="student-imported-view-bar" role="status">
            ...
            <button class="ghost-button" data-action="clear-student-imported-view" type="button">Показать всех</button>
          </div>
        ` : ""}
  ...
  function getStudentApplicationSourceKey(row) {
    ...
    return orderId && lineId ? `${sourceType}:${orderId}:${lineId}` : "";
  }

  function getStudentApplicationProgramTitle(row) {
  ...
  function renderStudentApplicationsImport() {
    ...
    const selectedPayment = selectedRows.reduce((sum, row) => sum + Number(row.paymentAmount || 0), 0);
        ...
        <section class="modal student-applications-import-modal" role="dialog" aria-modal="true" aria-label="Импорт слушателей">
          <form data-student-applications-import-form>
            <header class="modal-head student-applications-import-head">
              <div>
                <p class="eyebrow">Слушатели и заявки</p>
                <h2>Импорт слушателей</h2>
              </div>
            ...
            <div class="student-applications-import-filters">
              ...
              <label>
                ...
                <select name="period">
                  ...
                  <option value="custom" ${filters.period === "custom" ? "selected" : ""}>Произвольный период</option>
                </select>
              </label>
              <label>
                <span>Дата с</span>
              ...
              <label class="student-applications-paid-filter">
                ...
                <span>Только с оплатой</span>
              </label>
              <button class="primary-button student-applications-fetch-button ${importState.loading ? "is-loading" : ""}" type="submit" ${importState.loading ? "disabled" : ""}>
                ${importState.loading ? "Получение..." : "Получить список заявок"}
              </button>
            </div>

            <div class="student-applications-import-tools">
              <label class="search-box">
                <span>⌕</span>
            ...
            <div class="student-applications-import-messages">
              ...
              ${(importState.warnings || []).map((warning) => `<div class="student-applications-import-warning">${escapeHtml(warning)}</div>`).join("")}
              ${importState.truncated ? `<div class="student-applications-import-warning">Показаны первые 5000 заявок. Уточните условия отбора.</div>` : ""}
            </div>
            ...
            <div class="modal-actions student-applications-import-actions">
              <button class="ghost-button" data-action="close-student-applications-import" type="button">Закрыть</button>
              <button class="primary-button" data-action="add-selected-student-applications" type="button" ${selectedRows.length && !importState.importing ? "" : "disabled"}>
                ${importState.importing ? "Создание папок..." : "Добавить в базу"}
              </button>
            </div>
          </form>
        </section>
  ...
  function syncStudentApplicationsImportFilters(form) {
    ...
    filters.onlyPaid = Boolean(form.elements.onlyPaid?.checked);
    filters.search = String(form.elements.applicationSearch?.value || "");
    filters.status = String(form.elements.importStatus?.value || "На зачисление");
  }
  ...
  async function ensureStudentDocumentFolders(records) {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      throw new Error(payload.error || "Не удалось создать папки слушателей на Яндекс-Диске.");
    }
    const folderById = new Map((payload.folders || []).map((item) => [
      String(item.id),
  ...
  async function addSelectedStudentApplications() {
    ...
    selectedRows.forEach((row) => {
      ...
      imported.push(createStudentFromApplication(
        row,
        nextUid,
        status,
        state.studentApplicationsImport.filters.programId
    ...
    if (!imported.length) {
      alert("В выбранных заявках не заполнено ФИО.");
      return;
    }
  ...
  function renderDatabaseImportIndicator() {
    const progress = Math.max(0, Math.min(100, Number(state.databaseImport.progress || 0)));
    const tone = ["active", "success", "error"].includes(state.databaseImport.tone)
      ? state.databaseImport.tone
      : "active";
    return `
      <div class="database-import-indicator tone-${tone} ${state.databaseImport.indeterminate ? "is-indeterminate" : ""}" data-database-import-indicator role="status" aria-live="polite" ${state.databaseImport.visible ? "" : "hidden"}>
        <div class="database-import-indicator-head">
          <span class="database-import-indicator-icon" aria-hidden="true"></span>
          <div>
            <strong>Импорт из АИС Допобразование</strong>
            <span data-database-import-status>${escapeHtml(state.databaseImport.status || "Подготовка...")}</span>
          </div>
          <span class="database-import-indicator-percent" data-database-import-percent ${state.databaseImport.indeterminate || tone === "error" ? "hidden" : ""}>${progress}%</span>
        </div>
        <div class="database-import-progress-track" data-database-import-progress>
  ...
  function renderDatabaseExportIndicator() {
    const progress = Math.max(0, Math.min(100, Number(state.databaseExport.progress || 0)));
    const tone = ["active", "success", "error"].includes(state.databaseExport.tone)
      ? state.databaseExport.tone
      : "active";
    return `
      <div class="database-import-indicator tone-${tone} ${state.databaseExport.indeterminate ? "is-indeterminate" : ""}" data-database-export-indicator role="status" aria-live="polite" ${state.databaseExport.visible ? "" : "hidden"}>
        <div class="database-import-indicator-head">
          <span class="database-import-indicator-icon" aria-hidden="true"></span>
          <div>
            <strong>Синхронизация с АИС Допобразование</strong>
            <span data-database-export-status>${escapeHtml(state.databaseExport.status || "Подготовка...")}</span>
          </div>
          <span class="database-import-indicator-percent" data-database-export-percent ${state.databaseExport.indeterminate || tone === "error" ? "hidden" : ""}>${progress}%</span>
        </div>
        <div class="database-import-progress-track" data-database-export-progress>
          <span style="width: ${progress}%"></span>
        </div>
      </div>
    `;
  ...
  function renderStudentProgramTypeFilter() {
    const selected = STUDENT_PROGRAM_TYPE_FILTER_OPTIONS
    ...
    return `
      <details class="student-program-type-filter ${selected.length ? "is-active" : ""}" data-program-type-filter data-student-program-type-filter>
        ...
        <div class="student-program-type-filter-panel">
          ${STUDENT_PROGRAM_TYPE_FILTER_OPTIONS.map((item) => `
            <label>
              <input type="checkbox" value="${escapeAttr(item)}" data-student-program-type-filter-option ${selected.includes(item) ? "checked" : ""}>
  ...
  function updateDatabaseImportIndicator(patch) {
    ...
    const indicator = document.querySelector("[data-database-import-indicator]");
    if (indicator) {
      const tone = ["active", "success", "error"].includes(state.databaseImport.tone)
        ? state.databaseImport.tone
        : "active";
      indicator.hidden = !state.databaseImport.visible;
      indicator.className = `database-import-indicator tone-${tone}${state.databaseImport.indeterminate ? " is-indeterminate" : ""}`;
      const status = indicator.querySelector("[data-database-import-status]");
      if (status) status.textContent = state.databaseImport.status || "Подготовка...";
      const progress = Math.max(0, Math.min(100, Number(state.databaseImport.progress || 0)));
      const progressBar = indicator.querySelector("[data-database-import-progress] > span");
      if (progressBar) progressBar.style.width = `${progress}%`;
      const percent = indicator.querySelector("[data-database-import-percent]");
      if (percent) {
        percent.hidden = Boolean(state.databaseImport.indeterminate || tone === "error");
        percent.textContent = `${progress}%`;
      }
    }
    document.querySelectorAll("[data-action='import-students-database']").forEach((button) => {
  ...
  function finishDatabaseImportIndicator(tone, status, hideAfter = 3500) {
    ...
    databaseImportHideTimer = window.setTimeout(() => {
      updateDatabaseImportIndicator({ visible: false });
    }, hideAfter);
  }

  function updateDatabaseExportIndicator(patch) {
    window.clearTimeout(databaseExportHideTimer);
    state.databaseExport = { ...state.databaseExport, ...patch };
    const indicator = document.querySelector("[data-database-export-indicator]");
    if (indicator) {
      const tone = ["active", "success", "error"].includes(state.databaseExport.tone)
        ? state.databaseExport.tone
        : "active";
      indicator.hidden = !state.databaseExport.visible;
      indicator.className = `database-import-indicator tone-${tone}${state.databaseExport.indeterminate ? " is-indeterminate" : ""}`;
      const status = indicator.querySelector("[data-database-export-status]");
      if (status) status.textContent = state.databaseExport.status || "Подготовка...";
      const progress = Math.max(0, Math.min(100, Number(state.databaseExport.progress || 0)));
      const progressBar = indicator.querySelector("[data-database-export-progress] > span");
      if (progressBar) progressBar.style.width = `${progress}%`;
      const percent = indicator.querySelector("[data-database-export-percent]");
      if (percent) {
        percent.hidden = Boolean(state.databaseExport.indeterminate || tone === "error");
        percent.textContent = `${progress}%`;
      }
    }
  ...
  function finishDatabaseExportIndicator(tone, status, hideAfter = 4500) {
    updateDatabaseExportIndicator({
      running: false,
      visible: true,
      status,
      progress: tone === "success" ? 100 : state.databaseExport.progress,
      tone,
      indeterminate: false
    });
    databaseExportHideTimer = window.setTimeout(() => {
      updateDatabaseExportIndicator({ visible: false });
    }, hideAfter);
  }
  ...
  function getFilterOptions(config) {
    ...
    if (!key) return [];
    const values = unique(rows.map((row) => row[key]).filter(Boolean));
    if (rows.some((row) => !String(row[key] || "").trim())) values.push("Не задано");
    return values;
  }
  ...
  function getTablePagination(configId, totalRows) {
    const pageSize = getTablePageSize(configId);
    const totalPages = Math.max(1, Math.ceil(totalRows / pageSize));
    const requestedPage = Number(state.tablePages[configId] || 1);
    const page = Math.max(1, Math.min(totalPages, Number.isFinite(requestedPage) ? requestedPage : 1));
    state.tablePages[configId] = page;
  ...
  function renderTablePagination(configId, totalRows, pagination) {
    if (totalRows <= TABLE_PAGE_SIZE_OPTIONS[0]) return "";
    const { page, pageSize, totalPages, start, end } = pagination;
    return `
      <div class="table-pagination" data-table-pagination="${escapeAttr(configId)}">
        <span class="table-pagination-summary">Показано ${start + 1}–${end} из ${totalRows}</span>
        <div class="table-pagination-navigation" aria-label="Навигация по страницам">
          <button class="icon-button" data-action="table-page" data-config="${escapeAttr(configId)}" data-page="1" type="button" title="Первая страница" aria-label="Первая страница" ${page <= 1 ? "disabled" : ""}>«</button>
          <button class="icon-button" data-action="table-page" data-config="${escapeAttr(configId)}" data-page="${page - 1}" type="button" title="Предыдущая страница" aria-label="Предыдущая страница" ${page <= 1 ? "disabled" : ""}>‹</button>
          <label class="table-pagination-current">
            <span>Страница</span>
            <input data-action="table-page-input" data-config="${escapeAttr(configId)}" type="number" min="1" max="${totalPages}" value="${page}" aria-label="Номер страницы">
            <span>из ${totalPages}</span>
          </label>
          <button class="icon-button" data-action="table-page" data-config="${escapeAttr(configId)}" data-page="${page + 1}" type="button" title="Следующая страница" aria-label="Следующая страница" ${page >= totalPages ? "disabled" : ""}>›</button>
          <button class="icon-button" data-action="table-page" data-config="${escapeAttr(configId)}" data-page="${totalPages}" type="button" title="Последняя страница" aria-label="Последняя страница" ${page >= totalPages ? "disabled" : ""}>»</button>
        </div>
        <label class="table-page-size">
          <span>Строк на странице</span>
          <select data-action="table-page-size" data-config="${escapeAttr(configId)}">
            ${TABLE_PAGE_SIZE_OPTIONS.map((size) => `<option value="${size}" ${size === pageSize ? "selected" : ""}>${size}</option>`).join("")}
          </select>
        </label>
      </div>
    `;
  ...
  function getDocumentTemplateSourceLabel(item) {
    if (item.templateUrl && getDocumentTemplateWebDavPath(item.templateUrl)) return "WebDAV";
  ...
  function getDocumentTemplateWebDavPath(value) {
    const source = String(value || "").trim();
    if (!source) return "";
    if (/^webdav:\/\//i.test(source)) {
      try {
        ...
        return source.replace(/^webdav:\/\/(?:webdav\.yandex\.ru\/)?/i, "").replace(/^\/+|\/+$/g, "");
      }
    }
    if (/^https?:\/\//i.test(source)) {
      try {
        const parsed = new URL(source);
        if (parsed.hostname.toLowerCase() !== "webdav.yandex.ru") return "";
        return decodeURIComponent(parsed.pathname).replace(/^\/+|\/+$/g, "");
      } catch {
        return "";
      }
  ...
  function getYandexDiskResourceOpenUrl(source) {
    const resourcePath = getDocumentTemplateWebDavPath(source);
    if (!resourcePath) return "";
    const sourceParts = resourcePath.split("/").map((part) => part.trim()).filter(Boolean);
    const baseParts = getYandexDiskBasePath()
      .replace(/\\/g, "/")
      .split("/")
      .map((part) => part.trim())
      .filter(Boolean);
    const sourceLower = sourceParts.map((part) => part.toLocaleLowerCase("ru-RU"));
    const baseLower = baseParts.map((part) => part.toLocaleLowerCase("ru-RU"));
    const alreadyAbsolute = sourceLower[0] && sourceLower[0] === baseLower[0];
    const fullPath = alreadyAbsolute ? sourceParts : [...baseParts, ...sourceParts];
  ...
  function getDocumentTemplateOpenUrl(item) {
    ...
    if (templateUrl) return getYandexDiskResourceOpenUrl(templateUrl) || templateUrl;
  ...
  function renderDocumentTemplateLinkDialog() {
    ...
    return `
      <div class="modal-backdrop document-link-dialog-backdrop" data-action="close-document-template-link-dialog">
        <section class="modal document-link-dialog" role="dialog" aria-modal="true" aria-label="Добавить источник документа">
          ...
          <form class="document-link-form" data-action="submit-document-template-link">
            <label>
              <span>WebDAV-путь или ссылка на файл Word</span>
            ...
            <form class="sdo-settings-form" data-action="save-student-database-settings">
              ...
              <div class="sdo-settings-fields">
                ...
                <div class="admin-system-documents-head">
                  <strong>Документы системы на Яндекс-Диске (WebDAV)</strong>
                  <button class="ghost-button admin-connection-test-button" data-action="test-yandex-disk" type="button">Проверить WebDAV</button>
                </div>
                <label>
      ...
      <section class="form-section program-training-plan-section">
        ...
        <div class="program-training-plan-scroll">
          <div class="editable-grid program-training-plan-grid" data-program-training-plan-body data-next-index="${nextIndex}">
            ${renderProgramTrainingPlanHead()}
            ${rows.map((item, index) => renderProgramTrainingPlanRow(item, index)).join("")}
          </div>
        </div>
        ${rows.length ? "" : `<p class="payment-empty">Строк учебного плана пока нет. Добавьте их здесь или импортируйте лист «Учебные планы».</p>`}
      </section>
    `;
  }

  function renderProgramTrainingPlanHead() {
    const columns = [
      { label: "№" },
      { label: "Дисциплина" },
      { label: "Описание" },
      ...
      { label: "Практика" },
      { label: "Аттестация" },
      { label: "", toggleHidden: true },
      { label: "Преподаватель", hidden: true },
      { label: "Материалы", hidden: true },
      { label: "Содержание", hidden: true },
      { label: "" }
    ];
    return columns.map((item) => {
      if (item.toggleHidden) {
        return `
          <div class="editable-grid-head program-plan-toggle-head">
            <button class="program-plan-expand-button" data-action="toggle-program-training-plan-hidden-columns" type="button" title="Показать скрытые колонки" aria-label="Показать скрытые колонки">+</button>
          </div>
        `;
      }
      return `<div class="editable-grid-head ${item.hidden ? "program-plan-hidden-column" : ""}">${escapeHtml(item.label)}</div>`;
    }).join("");
  }
    ...
    return `
      <div class="editable-grid-row program-training-plan-row" data-program-training-plan-row>
        <div class="editable-grid-cell program-training-plan-number">
          <button class="program-plan-drag-handle" data-action="drag-program-training-plan-row" draggable="true" type="button" title="Перетащите строку" aria-label="Перетащить строку">
            <span></span><span></span><span></span><span></span><span></span><span></span>
          </button>
          <span data-plan-row-number>${rowNumber}</span>
          <input type="hidden" data-plan-field="id" name="trainingPlan_${index}_id" value="${escapeAttr(item.id || "")}">
          <input type="hidden" data-plan-field="code" name="trainingPlan_${index}_code" value="${escapeAttr(rowNumber)}">
        </div>
        ...
        <div class="editable-grid-cell program-plan-toggle-spacer"></div>
        ${renderProgramTrainingPlanInput(index, "teacher", item.teacher || "", "text", "program-plan-hidden-column")}
        ${renderProgramTrainingPlanInput(index, "materials", item.materials || "", "text", "program-plan-hidden-column")}
        ${renderProgramTrainingPlanInput(index, "content", item.content || "", "text", "program-plan-hidden-column")}
        <div class="editable-grid-cell">
          <div class="program-training-plan-actions">
            <button class="program-plan-move-button" data-action="move-program-training-plan-row" data-direction="-1" type="button" title="Переместить выше" aria-label="Переместить выше">↑</button>
            <button class="program-plan-move-button" data-action="move-program-training-plan-row" data-direction="1" type="button" title="Переместить ниже" aria-label="Переместить ниже">↓</button>
            <button class="payment-row-delete" data-action="delete-program-training-plan-row" type="button" title="Удалить строку" aria-label="Удалить строку">
              <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M4 7h16"></path><path d="M10 11v6"></path><path d="M14 11v6"></path><path d="M6 7l1 14h10l1-14"></path><path d="M9 7V4h6v3"></path></svg>
            </button>
          </div>
        </div>
  ...
  function renderProgramTrainingPlanAttestationSelect(index, value = "") {
    const selectedValue = String(value || "").trim();
    const dictionaryValues = Array.isArray(state.data.dictionaries.trainingPlanAttestationTypes)
      ? state.data.dictionaries.trainingPlanAttestationTypes
      : dictionaryDefaults.trainingPlanAttestationTypes;
          ...
          <form id="recordForm" data-config="students" data-id="${record.id || ""}">
            ...
            <div class="student-card-layout">
              <section class="student-card-main">
                ...
                <div class="student-tab-body ${activeTab.id === "main" ? "student-main-tab" : ""} ${activeTab.id === "documents" ? "student-documents-tab" : ""} ${activeTab.id === "income" ? "student-income-tab" : ""} ${activeTab.id === "communications" ? "student-communications-tab" : ""} ${activeTab.id === "ordersSdo" ? "student-orders-sdo-tab" : ""} ${activeTab.id === "results" ? "student-results-tab" : ""}">
  ...
  function ensureFormDateForGeneration(fieldName, message) {
    const formElement = document.getElementById("recordForm");
    const input = formElement?.elements[fieldName];
    const value = String(input?.value || "").trim();
    if (parseOrdersSdoDate(value)) return true;
    alert(message);
    input?.focus({ preventScroll: true });
    return false;
  }
  ...
  function generateEnrollmentOrderNumber() {
    if (!ensureFormDateForGeneration("enrollmentDate", "Заполните дату зачисления перед формированием номера приказа о зачислении.")) return;
    generateNumberFromDataFormula("enrollmentOrderNumber");
  }

  function generateExpulsionOrderNumber() {
    if (!ensureFormDateForGeneration("expulsionDate", "Заполните дату отчисления перед формированием номера приказа об отчислении.")) return;
      ...
      <label class="student-photo-path-field" title="${escapeAttr(tooltip)}">
        <div class="student-photo-path-control">
          ...
          <button
            ...
            type="button"
            title="Создать папку документов и заполнить путь, если он пуст"
            aria-label="Создать папку документов и заполнить путь, если он пуст"
          >
            ${renderOrdersSdoIcon("wand")}
          </button>
        </div>
  ...
  function getProgramTrainingPlanNameKeys(programRecord = {}, fallbackName = "") {
    const names = [fallbackName, programRecord.name, programRecord.shortName]
      .map((value) => String(value || "").trim())
      .filter(Boolean);
    return {
      exact: new Set(names.map(normalizeProgramName).filter(Boolean)),
      comparable: new Set(names.map(normalizeTrainingPlanProgramName).filter(Boolean))
    };
  }

  function isTrainingPlanRowLinkedToProgram(item, programRecord = {}, fallbackName = "") {
    const programId = String(programRecord.id || "").trim();
    const rowProgramId = String(item?.programId || "").trim();
    if (programId && rowProgramId) return rowProgramId === programId;
  ...
  function renderPaymentRows(record) {
    ...
    return `
      <section class="form-section">
        ...
        ${rowIndexes.length ? `<div class="editable-grid payment-grid" data-student-finance-grid="payments">
          ...
          ${rowIndexes.map((n) => {
            ...
            return `
              <div class="editable-grid-row payment-grid-row" data-student-finance-row data-finance-row-key="${n}">
                <div class="editable-grid-cell row-number-cell finance-row-number-cell" data-label="№">
                  ${renderFinanceRowDragHandle(`Перетащить оплату ${n}`)}
                  <strong class="editable-grid-row-number" data-finance-row-number>${n}</strong>
                </div>
  ...
  function getStudentExpenseTypeOptions(record = {}) {
    ...
    return unique([
      ...
      ...studentTypes,
      ...currentTypes,
      ...(state.data.collections.students || []).flatMap((student) => (
        Array.isArray(student.directExpenses) ? student.directExpenses.map((expense) => expense.type) : []
      )),
  ...
  function getInventoryBalance(item) {
  ...
  function findInventoryItemById(id) {
    ...
    return (state.data.collections.inventory || []).find((item) => String(item.id || "") === inventoryId) || null;
  }

  function getStudentExpenseInventoryOptions(selectedId = "") {
    const currentId = String(selectedId || "").trim();
    return (state.data.collections.inventory || [])
      .filter((item) => (
        String(item.id || "").trim()
        && String(item.itemType || "").trim()
        && (getInventoryBalance(item) > 0 || String(item.id || "") === currentId)
      ))
      .slice()
      .sort((left, right) => (
        String(left.itemType || "").localeCompare(String(right.itemType || ""), "ru", {
          numeric: true,
          sensitivity: "base"
        })
  ...
  function renderStudentExpenseTypeSelect({
    prefix,
    type,
    inventoryId,
    inventoryLink,
      ...
      >
        ...
        <optgroup label="Виды затрат">
          ${typeOptions.map((option, optionIndex) => `
            <option
              value="type:${optionIndex}"
              data-expense-type="${escapeAttr(option)}"
              ${!selectedInventoryId && option === type ? "selected" : ""}
            >${escapeHtml(option)}</option>
          `).join("")}
        </optgroup>
        ${inventoryOptions.length || missingInventoryOption ? `
          <optgroup label="Запасы">
            ${missingInventoryOption}
            ${inventoryOptions.map((item) => {
              const itemId = String(item.id || "");
              const selected = itemId === selectedInventoryId;
              return `
                <option
                  value="inventory:${escapeAttr(itemId)}"
                  data-inventory-id="${escapeAttr(itemId)}"
                  ${selected ? "selected" : ""}
                  ${!selected && getInventoryBalance(item) <= 0 ? "disabled" : ""}
                >${escapeHtml(formatStudentExpenseInventoryOption(item))}</option>
              `;
            }).join("")}
          </optgroup>
        ` : ""}
      </select>
      <input name="${prefix}Type" type="hidden" value="${escapeAttr(type || "")}">
      <input name="${prefix}InventoryId" type="hidden" value="${escapeAttr(selectedInventoryId)}">
      <input name="${prefix}InventoryLink" type="hidden" value="${escapeAttr(inventoryLink || selectedInventory?.itemType || "")}">
    `;
  }
  ...
  function applyStudentExpenseInventoryChoice(select) {
    ...
    const inventoryIdInput = form.elements[`${prefix}InventoryId`];
    const inventoryLinkInput = form.elements[`${prefix}InventoryLink`];
    const typeInput = form.elements[`${prefix}Type`];
    const amountInput = form.elements[`${prefix}Amount`];
    const inventoryId = String(option?.dataset.inventoryId || "").trim();
    if (inventoryId) {
      const inventoryItem = findInventoryItemById(inventoryId);
      if (!inventoryItem) return;
      const previousInventoryId = String(inventoryIdInput?.value || "").trim();
      if (inventoryId !== previousInventoryId && getInventoryBalance(inventoryItem) <= 0) {
        alert("Эта позиция запаса закончилась.");
        select.value = previousInventoryId ? `inventory:${previousInventoryId}` : "";
        return;
      }
      if (typeInput) typeInput.value = String(inventoryItem.itemType || "");
      if (inventoryIdInput) inventoryIdInput.value = inventoryId;
      if (inventoryLinkInput) inventoryLinkInput.value = String(inventoryItem.itemType || "");
      if (amountInput) amountInput.value = String(normalizeOptionalNumber(inventoryItem.amount) || 0);
    } else {
      if (typeInput) typeInput.value = String(option?.dataset.expenseType || "");
  ...
  function getStudentInventoryAllocationCounts(record = {}) {
    ...
    const add = (inventoryId) => {
      const id = String(inventoryId || "").trim();
      if (id) counts.set(id, (counts.get(id) || 0) + 1);
    };
    Array.from({ length: 6 }, (_, index) => index + 1).forEach((index) => {
      if (expenseRowHasData(record, index)) add(record[`expense${index}InventoryId`]);
    });
    (Array.isArray(record.directExpenses) ? record.directExpenses : []).forEach((expense) => {
      add(expense?.inventoryId);
    });
    return counts;
  }

  function applyStudentInventoryAllocationChanges(previousRecord = {}, nextRecord = {}) {
    const previousCounts = getStudentInventoryAllocationCounts(previousRecord);
    const nextCounts = getStudentInventoryAllocationCounts(nextRecord);
    const ids = new Set([...previousCounts.keys(), ...nextCounts.keys()]);
    const changes = [...ids].map((id) => ({
      id,
      delta: (nextCounts.get(id) || 0) - (previousCounts.get(id) || 0)
    })).filter((change) => change.delta);
    for (const change of changes) {
      if (change.delta <= 0) continue;
      const item = findInventoryItemById(change.id);
      if (!item) {
        alert("Выбранная позиция запаса больше не существует. Обновите строку расхода.");
        return false;
      }
      if (getInventoryBalance(item) < change.delta) {
        alert(`Недостаточно запаса «${item.itemType || change.id}». Доступно: ${getInventoryBalance(item)}.`);
        return false;
      }
    }
    changes.forEach((change) => {
      const item = findInventoryItemById(change.id);
      if (!item) return;
      item.balance = Math.max(0, getInventoryBalance(item) - change.delta);
    });
    return true;
  }

  function getInventoryAllocationCount(inventoryId) {
    const id = String(inventoryId || "").trim();
    if (!id) return 0;
    const studentCount = (state.data.collections.students || []).reduce((total, student) => (
      total + (getStudentInventoryAllocationCounts(student).get(id) || 0)
    ), 0);
    const unlinkedCount = (state.data.collections.directExpenses || [])
      .filter((expense) => String(expense?.inventoryId || "").trim() === id)
      .length;
    return studentCount + unlinkedCount;
  }
  ...
  function getStudentExpenseNoteOptions(record = {}) {
    ...
    const linkedDirectExpenseNotes = (state.data.collections.students || []).flatMap((student) => (
      Array.isArray(student.directExpenses) ? student.directExpenses.map((expense) => expense.note) : []
    ));
  ...
  function expenseRowHasData(record, index) {
    return Boolean(
      ...
      record[`expense${index}InventoryId`]
    );
  }
  ...
  function renderExpenseRows(record) {
    ...
    const manualRowIndexes = getVisibleExpenseRowIndexes(record);
    const directExpenses = Array.isArray(record.directExpenses) ? record.directExpenses : [];
    const rowCount = manualRowIndexes.length + directExpenses.length;
    return `
      <section class="form-section">
        <div class="payment-section-head">
          <h3>Расходы</h3>
          ...
          ${manualRowIndexes.map((n, rowIndex) => {
            ...
            return `
              <div class="editable-grid-row expense-grid-row" data-student-finance-row data-finance-row-key="manual-${n}">
                <div class="editable-grid-cell row-number-cell finance-row-number-cell" data-label="№">
                  ${renderFinanceRowDragHandle(`Перетащить расход ${rowIndex + 1}`)}
                  <strong class="editable-grid-row-number" data-finance-row-number>${rowIndex + 1}</strong>
                </div>
                <label class="editable-grid-cell" data-label="Дата">
                ...
                <label class="editable-grid-cell" data-label="Вид затрат">
                  ${renderStudentExpenseTypeSelect({
                    prefix: `expense${n}`,
                    type,
                    inventoryId: record[`expense${n}InventoryId`],
                    inventoryLink: record[`expense${n}InventoryLink`],
                    typeOptions,
                    index: n
                  })}
                </label>
          ...
          ${directExpenses.map((expense, index) => {
            ...
            return `
              <div class="editable-grid-row expense-grid-row" data-student-finance-row data-finance-row-key="direct-${index}">
                <div class="editable-grid-cell row-number-cell finance-row-number-cell" data-label="№">
                  ${renderFinanceRowDragHandle(`Перетащить расход ${rowNumber}`)}
                  <strong class="editable-grid-row-number" data-finance-row-number>${rowNumber}</strong>
                </div>
                <label class="editable-grid-cell" data-label="Дата">
                  <input name="directExpense${index}Date" data-direct-expense-index="${index}" type="date" value="${escapeAttr(expense.date || "")}">
                </label>
                <label class="editable-grid-cell" data-label="Вид затрат">
                  ${renderStudentExpenseTypeSelect({
                    prefix: `directExpense${index}`,
                    type,
                ...
                <label class="editable-grid-cell" data-label="Сумма">
                  <input name="directExpense${index}Amount" data-direct-expense-index="${index}" type="number" value="${escapeAttr(expense.amount || "")}">
                </label>
                <div class="editable-grid-cell expense-note-cell" data-label="Примечание">
                  ${renderComboField({
                    name: `directExpense${index}Note`,
                    type: "search",
                    value: expense.note || "",
                    options: noteOptions,
                ...
                <div class="editable-grid-cell row-action-cell expense-row-actions" data-label="Действия">
                  ...
                  <button
                    ...
                    type="button"
                    title="Удалить расход"
                    aria-label="Удалить расход ${rowNumber}"
                  >
                    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
                      <path d="M3 6h18"></path>
                      <path d="M8 6V4h8v2"></path>
  ...
  function compactStudentExpenses(record, deletedIndex) {
    const expenses = Array.from({ length: 6 }, (_, index) => index + 1)
      ...
      .map((index) => ({
        ...
        inventoryId: record[`expense${index}InventoryId`] || "",
        inventoryLink: record[`expense${index}InventoryLink`] || ""
      }))
      .filter((expense) => (
        expense.date
        || expense.type
    ...
    Array.from({ length: 6 }, (_, index) => index + 1).forEach((index) => {
      ...
      record[`expense${index}InventoryId`] = expense.inventoryId || "";
      record[`expense${index}InventoryLink`] = expense.inventoryLink || "";
    });
    return record;
  }
  ...
  function clearStudentDirectExpenseRow(index) {
    if (!state.modal) return;
    const draft = collectStudentFormDraft();
    const directExpenses = Array.isArray(draft.directExpenses) ? draft.directExpenses : [];
    const expenseIndex = Number(index);
    if (!Number.isInteger(expenseIndex) || !directExpenses[expenseIndex]) return;
    if (!confirm("Удалить запись расхода?")) return;
    const nextDraft = {
      ...draft,
      directExpenses: directExpenses.filter((expense, currentIndex) => currentIndex !== expenseIndex)
    };
    nextDraft.expenseTotal = sumStudentExpenses(nextDraft);
    ...
    state.modal.draft = {
      ...
      directExpenses: [
        ...directExpenses,
        {
          id: makeId("direct-expense"),
          uid: draft.uid || "",
          date: formatOrdersSdoDate(new Date()),
          type: "",
          amount: "",
          isPaid: "",
          note: "",
  ...
  function reorderStudentPayments(rowKeys) {
    if (!state.modal) return;
    const draft = collectStudentFormDraft();
    const payments = rowKeys.map((key) => {
      const index = Number(key);
      if (!Number.isInteger(index) || index < 1 || index > 8) return null;
      return {
        date: draft[`payment${index}Date`] || "",
        amount: draft[`payment${index}Amount`] || "",
        note: draft[`payment${index}Note`] || ""
      };
    }).filter(Boolean);
    Array.from({ length: 8 }, (_, index) => index + 1).forEach((index) => {
      const payment = payments[index - 1] || {};
      draft[`payment${index}Date`] = payment.date || "";
      draft[`payment${index}Amount`] = payment.amount || "";
      draft[`payment${index}Note`] = payment.note || "";
    });
    state.openPaymentRows = payments
      .map((payment, index) => paymentRowHasData(draft, index + 1) ? "" : index + 1)
      .filter(Boolean);
  ...
  function reorderStudentExpenses(rowKeys) {
    if (!state.modal) return;
    const draft = collectStudentFormDraft();
    const directExpenses = Array.isArray(draft.directExpenses) ? draft.directExpenses : [];
    const orderedExpenses = rowKeys.map((key) => {
      const directMatch = /^direct-(\d+)$/.exec(key);
      if (directMatch) return directExpenses[Number(directMatch[1])] || null;
      const manualMatch = /^manual-(\d+)$/.exec(key);
      if (!manualMatch) return null;
      const index = Number(manualMatch[1]);
      if (!expenseRowHasData(draft, index)) return null;
      return {
        id: makeId("direct-expense"),
        uid: draft.uid || "",
        date: draft[`expense${index}Date`] || "",
        type: draft[`expense${index}Type`] || "",
        amount: draft[`expense${index}Amount`] || "",
        isPaid: draft[`expense${index}IsPaid`] || "",
        note: draft[`expense${index}Note`] || "",
        inventoryId: draft[`expense${index}InventoryId`] || "",
        inventoryLink: draft[`expense${index}InventoryLink`] || ""
      };
    }).filter(Boolean);
    Array.from({ length: 6 }, (_, index) => index + 1).forEach((index) => {
      draft[`expense${index}Date`] = "";
      draft[`expense${index}Type`] = "";
      draft[`expense${index}Amount`] = "";
      draft[`expense${index}IsPaid`] = "";
      draft[`expense${index}Note`] = "";
      draft[`expense${index}InventoryId`] = "";
      draft[`expense${index}InventoryLink`] = "";
    });
    draft.directExpenses = orderedExpenses;
    draft.expenseTotal = sumStudentExpenses(draft);
    state.openExpenseRows = [];
  ...
  function getStudentFinanceDragAfterRow(grid, y) {
    return [...grid.querySelectorAll("[data-student-finance-row]:not(.is-dragging)")].reduce((closest, row) => {
      const cell = row.querySelector(".editable-grid-cell");
      if (!cell) return closest;
      const box = cell.getBoundingClientRect();
      const offset = y - box.top - box.height / 2;
      return offset < 0 && offset > closest.offset ? { offset, row } : closest;
  ...
  function updateStudentFinanceRowNumbers(grid) {
    grid.querySelectorAll("[data-student-finance-row]").forEach((row, index) => {
      const number = row.querySelector("[data-finance-row-number]");
      if (number) number.textContent = String(index + 1);
    });
  }

  function bindStudentFinanceRowDrag(grid) {
    if (!grid) return;
    grid.addEventListener("dragstart", (event) => {
      const handle = event.target.closest("[data-finance-row-drag]");
      const row = handle?.closest("[data-student-finance-row]");
      if (!row) {
        event.preventDefault();
        return;
      }
      grid.dataset.originalFinanceOrder = [...grid.querySelectorAll("[data-student-finance-row]")]
        .map((item) => item.dataset.financeRowKey)
        .join("|");
      row.classList.add("is-dragging");
      if (event.dataTransfer) {
        event.dataTransfer.effectAllowed = "move";
    ...
    grid.addEventListener("dragover", (event) => {
      const dragging = grid.querySelector("[data-student-finance-row].is-dragging");
      if (!dragging) return;
      event.preventDefault();
      const afterRow = getStudentFinanceDragAfterRow(grid, event.clientY);
      if (afterRow) grid.insertBefore(dragging, afterRow);
      else grid.appendChild(dragging);
      updateStudentFinanceRowNumbers(grid);
    });
    grid.addEventListener("drop", (event) => event.preventDefault());
    grid.addEventListener("dragend", () => {
      const rows = [...grid.querySelectorAll("[data-student-finance-row]")];
      rows.forEach((row) => row.classList.remove("is-dragging"));
      updateStudentFinanceRowNumbers(grid);
      const rowKeys = rows.map((row) => row.dataset.financeRowKey);
      const originalOrder = grid.dataset.originalFinanceOrder || "";
      delete grid.dataset.originalFinanceOrder;
      if (rowKeys.join("|") === originalOrder) return;
      if (grid.dataset.studentFinanceGrid === "payments") reorderStudentPayments(rowKeys);
      if (grid.dataset.studentFinanceGrid === "expenses") reorderStudentExpenses(rowKeys);
    });
  }
  ...
  function bindGlobalEscapeKey() {
    ...
    document.addEventListener("keydown", handleGlobalEscapeKey, { capture: true });
  }

  function handleGlobalEscapeKey(event) {
    if (event.key !== "Escape" || event.defaultPrevented || event.isComposing) return;
    if (!closeTopmostWindowByEscape()) return;
    event.preventDefault();
    event.stopPropagation();
  }
  ...
  function closeTopmostWindowByEscape() {
    ...
    if (state.documentTemplateLinkDialog) {
      closeDocumentTemplateLinkDialog();
      return true;
    }
    if (state.discountPickerOpen) {
      closeDiscountPicker();
      return true;
    }
    ...
    if (state.studentApplicationsImport.open) {
      closeStudentApplicationsImport();
      return true;
    }
    if (state.modal) {
  ...
  function bindEvents() {
    ...
    document.querySelector("[data-action='toggle-sidebar']")?.addEventListener("click", () => {
      ...
      if (document.body.classList.contains("sidebar-collapsed")) {
        ...
        document.body.classList.toggle("sidebar-open", isCompact);
        return;
      }
      if (isCompact) {
    ...
    programTrainingPlanBody?.addEventListener("click", (event) => {
      const toggleHiddenButton = event.target.closest("[data-action='toggle-program-training-plan-hidden-columns']");
      if (toggleHiddenButton) toggleProgramTrainingPlanHiddenColumns(toggleHiddenButton);
    });
    programTrainingPlanBody?.addEventListener("click", (event) => {
      const deleteButton = event.target.closest("[data-action='delete-program-training-plan-row']");
      if (deleteButton) {
        deleteProgramTrainingPlanRow(deleteButton);
        return;
      }
      const moveButton = event.target.closest("[data-action='move-program-training-plan-row']");
      if (moveButton) {
        moveProgramTrainingPlanRow(moveButton, Number(moveButton.dataset.direction || 0));
        return;
      }
    });
    ...
    document.querySelectorAll("[data-direct-expense-index][name]").forEach((input) => {
      input.addEventListener("input", syncStudentFinanceFields);
      input.addEventListener("change", syncStudentFinanceFields);
    });
    ...
    document.querySelectorAll("[data-student-program-type-filter-option]").forEach((input) => {
      input.addEventListener("change", () => {
        state.studentProgramTypeFilter = Array.from(
          document.querySelectorAll("[data-student-program-type-filter-option]:checked")
        ).map((item) => item.value);
        state.tablePages.students = 1;
        render();
        const filter = document.querySelector("[data-student-program-type-filter]");
        if (filter) filter.open = true;
      });
    });
    document.querySelector("[data-action='clear-student-program-type-filter']")?.addEventListener("click", () => {
      state.studentProgramTypeFilter = [];
      state.tablePages.students = 1;
      render();
    });
    ...
    document.querySelectorAll("[data-action='table-page-input']").forEach((input) => {
      const applyPage = () => {
        state.tablePages[input.dataset.config] = Number(input.value || 1);
        render();
      };
      input.addEventListener("change", applyPage);
    ...
    document.querySelectorAll("[data-action='table-page-size']").forEach((select) => {
      select.addEventListener("change", () => {
        const configId = select.dataset.config;
        const pageSize = Number(select.value);
        if (!TABLE_PAGE_SIZE_OPTIONS.includes(pageSize)) return;
        updateTableSettings(configId, (settings) => ({ ...settings, pageSize }));
        state.tablePages[configId] = 1;
        render();
      });
    });
    ...
    document.querySelectorAll(".mini-table-link-row").forEach((row) => {
      row.addEventListener("keydown", (event) => {
    ...
    document.getElementById("studentPhotoPath")?.addEventListener("change", (event) => {
      ...
      syncStudentPhotoPathUi(event.currentTarget.value, true);
    });
  ...
  function syncNavItemOrderFromDom() {
    const order = [...document.querySelectorAll("[data-nav-item]")]
  ...
  function getNavItemDragAfterElement(container, y) {
    return [...container.querySelectorAll("[data-nav-item]:not(.is-dragging)")].reduce((closest, child) => {
  ...
  function closeNavItemMenu() {
    document.removeEventListener("pointerdown", closeNavItemMenuOnOutsideClick, { capture: true });
    document.querySelector("[data-nav-item-menu]")?.remove();
  }

  function closeNavItemMenuOnOutsideClick(event) {
    if (event.target.closest("[data-nav-item-menu]")) {
  ...
  function showNavItemMenu(x, y, viewId) {
    closeNavItemMenu();
    const startsHere = loadStartView() === viewId;
    const menu = document.createElement("div");
    menu.className = "student-tab-menu nav-item-menu";
    menu.dataset.navItemMenu = "";
    menu.innerHTML = `
      <button class="nav-item-menu-option" data-action="set-start-view" data-view-id="${escapeAttr(viewId)}" type="button" aria-pressed="${startsHere}">
        <span class="nav-item-menu-check" aria-hidden="true">${startsHere ? "✓" : ""}</span>
        <span>Начинать с этого раздела</span>
      </button>
      <button data-action="reset-nav-item-order" type="button">Восстановить исходный порядок</button>
    `;
    ...
    menu.querySelector("[data-action='set-start-view']")?.addEventListener("click", (event) => {
      setStartView(event.currentTarget.dataset.viewId);
    });
    menu.querySelector("[data-action='reset-nav-item-order']")?.addEventListener("click", resetNavItemOrder);
    setTimeout(() => {
      document.addEventListener("pointerdown", closeNavItemMenuOnOutsideClick, { capture: true, once: true });
    });
  }

  function bindNavItemOrderControls() {
    ...
    container.addEventListener("contextmenu", (event) => {
      ...
      if (!item) return;
      event.preventDefault();
      showNavItemMenu(event.clientX, event.clientY, item.dataset.navItem);
    });
    container.addEventListener("dragover", (event) => {
      event.preventDefault();
      const dragging = container.querySelector(".is-dragging[data-nav-item]");
      if (!dragging) return;
      const afterElement = getNavItemDragAfterElement(container, event.clientY);
      if (afterElement) {
    ...
    container.querySelectorAll("[data-nav-item]").forEach((button) => {
      button.addEventListener("dragstart", (event) => {
        closeNavItemMenu();
        draggedNavItemId = button.dataset.navItem || "";
        event.dataTransfer.effectAllowed = "move";
        event.dataTransfer.setData("text/plain", draggedNavItemId);
        button.classList.add("is-dragging");
        document.body.classList.add("nav-item-dragging");
      });
      button.addEventListener("dragend", () => {
        button.classList.remove("is-dragging");
        document.body.classList.remove("nav-item-dragging");
        if (draggedNavItemId) button.dataset.wasDragged = "true";
  ...
  function collectStudentDirectExpenses(formElement, record = {}) {
    const directExpenses = Array.isArray(record.directExpenses)
      ...
      : [];
    return directExpenses.map((expense, index) => {
      const field = (suffix) => formElement?.elements?.[`directExpense${index}${suffix}`];
      const dateInput = field("Date");
      if (!dateInput) return expense;
      const typeInput = field("Type");
      const amountInput = field("Amount");
      const isPaidInput = field("IsPaid");
      ...
      return {
        ...
        isPaid: isPaidInput ? (isPaidInput.checked ? "Да" : "") : expense.isPaid || "",
  ...
  function addProgramTrainingPlanRow() {
    const body = document.querySelector("[data-program-training-plan-body]");
    if (!body) return;
    const index = Number(body.dataset.nextIndex || body.querySelectorAll("[data-program-training-plan-row]").length || 0);
    body.insertAdjacentHTML("beforeend", renderProgramTrainingPlanRow({ code: index + 1 }, index));
  ...
  function toggleProgramTrainingPlanHiddenColumns(button) {
    ...
    if (!body) return;
    const expanded = !body.classList.contains("show-hidden-columns");
    body.classList.toggle("show-hidden-columns", expanded);
    body.querySelectorAll("[data-action='toggle-program-training-plan-hidden-columns']").forEach((item) => {
      item.textContent = expanded ? "−" : "+";
      item.title = expanded ? "Скрыть дополнительные колонки" : "Показать скрытые колонки";
  ...
  function moveProgramTrainingPlanRow(button, direction) {
    const row = button?.closest("[data-program-training-plan-row]");
    const body = row?.closest("[data-program-training-plan-body]");
    if (!row || !body || !direction) return;
    const rows = [...body.querySelectorAll("[data-program-training-plan-row]")];
    ...
    if (direction < 0) {
      ...
      body.insertBefore(target, row);
    }
    updateProgramTrainingPlanRowNumbers(body);
    row.querySelector("[data-action='move-program-training-plan-row']")?.focus({ preventScroll: true });
  }

  function getProgramTrainingPlanDragAfterRow(body, y) {
    return [...body.querySelectorAll("[data-program-training-plan-row]:not(.is-dragging)")].reduce((closest, row) => {
      const cell = row.querySelector(".editable-grid-cell");
      if (!cell) return closest;
      const box = cell.getBoundingClientRect();
      const offset = y - box.top - box.height / 2;
  ...
  function bindProgramTrainingPlanRowDrag(body) {
    if (!body) return;
    body.addEventListener("dragstart", (event) => {
      const handle = event.target.closest("[data-action='drag-program-training-plan-row']");
      const row = handle?.closest("[data-program-training-plan-row]");
      if (!row) {
        event.preventDefault();
        return;
      }
    ...
    body.addEventListener("dragend", () => {
      body.querySelectorAll("[data-program-training-plan-row]").forEach((row) => row.classList.remove("is-dragging"));
      updateProgramTrainingPlanRowNumbers(body);
    });
    body.addEventListener("dragover", (event) => {
      const dragging = body.querySelector("[data-program-training-plan-row].is-dragging");
      if (!dragging) return;
      event.preventDefault();
      const afterRow = getProgramTrainingPlanDragAfterRow(body, event.clientY);
      if (afterRow) {
        body.insertBefore(dragging, afterRow);
      } else {
  ...
  function updateProgramTrainingPlanRowNumbers(body) {
    const rows = [...(body || document).querySelectorAll("[data-program-training-plan-row]")];
    rows.forEach((row, index) => {
      const number = row.querySelector("[data-plan-row-number]");
      if (number) number.textContent = String(index + 1);
      row.querySelectorAll("[data-plan-field]").forEach((input) => {
        const key = input.dataset.planField;
        if (key) input.name = `trainingPlan_${index}_${key}`;
      });
      const codeInput = row.querySelector('[data-plan-field="code"]');
      if (codeInput) codeInput.value = String(index + 1);
      const moveUp = row.querySelector("[data-action='move-program-training-plan-row'][data-direction='-1']");
      const moveDown = row.querySelector("[data-action='move-program-training-plan-row'][data-direction='1']");
      if (moveUp) moveUp.disabled = index === 0;
      if (moveDown) moveDown.disabled = index === rows.length - 1;
    });
  }

  function collectProgramTrainingPlanRows(formElement, programId, programValues) {
    const programName = String(programValues?.name || "").trim();
    return [...formElement.querySelectorAll("[data-program-training-plan-row]")]
      .map((row, index) => {
        const values = {};
        row.querySelectorAll("[data-plan-field]").forEach((input) => {
          const key = input.dataset.planField;
        ...
        return {
          id: String(values.id || "").trim() || makeId("trainingPlans"),
          programId,
          programName,
          code: String(index + 1),
          ...
          attestation: String(values.attestation || "").trim(),
          teacher: String(values.teacher || "").trim(),
          materials: String(values.materials || "").trim(),
  ...
  function syncProgramTrainingPlanRows(formElement, programId, programValues, previousProgramValues = {}) {
    if (!formElement || formElement.dataset.config !== "programs" || !programId) return;
    const rows = state.data.collections.trainingPlans || [];
    const currentProgram = { ...(programValues || {}), id: programId };
    const previousProgram = { ...(previousProgramValues || {}), id: programId };
    const relatedIds = new Set();
    rows.forEach((item) => {
      if (item?.id && (
        isTrainingPlanRowLinkedToProgram(item, currentProgram)
        || isTrainingPlanRowLinkedToProgram(item, previousProgram)
      )) {
    ...
    if (!inserted) nextRows.push(...submittedRows);
  ...
  async function handleStudentPhoto(event) {
    ...
    const studentName = String(document.querySelector("[name='name']")?.value || "").trim();
    if (!studentName) {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        console.warn("Не удалось сохранить фото на Яндекс-Диске", error);
        alert(`Не удалось сохранить фото на Яндекс-Диске: ${error.message}`);
      } finally {
  ...
  function getStudentPhotoPathLink(value, storedUrl = "") {
    const source = String(value || "").trim();
    if (!source) return "";
    if (isStudentSourcePhotoPath(source)) return studentSourcePhotoUrl(source);
    const candidate = String(storedUrl || source).trim();
  ...
  function studentDocumentsPathHasData(pathInput) {
    return Boolean(
      String(pathInput?.value || "").trim()
      || String(document.getElementById("studentPhotoData")?.value || "").trim()
      || String(document.getElementById("studentPhotoUrl")?.value || "").trim()
    );
  }

  async function createMissingStudentDocumentsFolder(event) {
    const button = event.currentTarget;
    const form = button.closest("#recordForm");
    const pathInput = form?.elements.photoPath;
    if (!form || form.dataset.config !== "students" || !(pathInput instanceof HTMLInputElement)) return;
    if (studentDocumentsPathHasData(pathInput)) {
      alert("Путь или данные фото уже заполнены. Существующие данные не изменены.");
      pathInput.focus({ preventScroll: true });
      return;
    }
    const studentName = String(form.elements.name?.value || "").trim();
    if (!studentName) {
      alert("Сначала укажите ФИО слушателя.");
    ...
    try {
      const folderRecord = {
        id: String(form.dataset.id || state.modal?.id || "current"),
        name: studentName
      };
      await ensureStudentDocumentFolders([folderRecord]);
      if (studentDocumentsPathHasData(pathInput)) {
        alert("За время создания папки путь был заполнен. Существующие данные не изменены.");
        return;
      }
      pathInput.value = folderRecord.photoPath;
      syncStudentPhotoPathUi(folderRecord.photoPath, false);
      state.modal.draft = collectStudentFormDraft();
      state.modal.hasDraftChanges = true;
      pathInput.focus({ preventScroll: true });
    } catch (error) {
      alert(`Не удалось создать папку документов: ${error.message}`);
    } finally {
      button.disabled = false;
      button.removeAttribute("aria-busy");
    }
  ...
  function syncStudentPhotoPathUi(value, refreshPreview = false) {
    const photoPath = String(value || "").trim();
    const storedUrl = document.getElementById("studentPhotoUrl")?.value || "";
    const photoUrl = getStudentPhotoPathLink(photoPath, storedUrl);
    const currentRecord = (state.data.collections.students || [])
      .find((item) => item.id === state.modal?.id) || {};
    const folderRecord = {
      ...currentRecord,
      ...(state.modal?.draft || {}),
      photoPath,
      name: document.querySelector("[name='name']")?.value || currentRecord.name || "",
      applicationDate: document.querySelector("[name='applicationDate']")?.value
        || currentRecord.applicationDate
        || "",
      startDate: document.querySelector("[name='startDate']")?.value || currentRecord.startDate || ""
    };
  ...
  async function uploadStoredPhoto(dataUrl, previousPath = "", meta = {}) {
    ...
    return {
      ...payload,
      photoUrl: isStudentSourcePhotoPath(payload.photoPath)
        ? studentSourcePhotoUrl(payload.photoPath)
        : photoPublicUrl(payload.photoUrl || payload.photoPath)
    };
  }
  ...
  async function bulkDelete(configId) {
    ...
    if (configId === "inventory") {
      ...
      if (allocated) {
        alert(`Позицию «${allocated.itemType || allocated.id}» нельзя удалить: она распределена на слушателей.`);
        return;
      }
    }
  ...
  function updateDocumentTemplateSearch(value, cursor = null) {
    ...
    state.tablePages.documentTemplates = 1;
    render();
  ...
  function buildDocumentTemplateTitleFromSource(value) {
    const source = String(value || "").trim();
    try {
      const parsed = new URL(source);
      ...
      const last = source.replace(/\\/g, "/").split("/").filter(Boolean).pop() || "";
      return stripWordTemplateExtension(last) || "Новый документ";
    }
  }
  ...
  async function addDocumentTemplateByLink(event = null) {
    ...
    const source = normalizeDocumentTemplateSource(urlInput?.value || "");
    if (!source) {
      alert("Укажите корректный WebDAV-путь или ссылку на файл Word.");
      urlInput?.focus();
      return;
    }
    ...
    try {
      const { documents, document } = collectContractTemplateForm();
      const inspection = await inspectDocumentTemplateSource({ templateUrl: source });
      const title = buildDocumentTemplateTitleFromSource(source);
  ...
  async function replaceActiveDocumentTemplateLink(event = null) {
    event?.preventDefault?.();
    const { documents, document } = collectContractTemplateForm();
    const input = window.prompt("Укажите WebDAV-путь или ссылку на файл Word", document.templateUrl || "");
    if (input === null) return;
    const source = normalizeDocumentTemplateSource(input);
    if (!source) {
      alert("Укажите корректный WebDAV-путь или ссылку на файл Word.");
      return;
    }
  ...
  function getStudentDatabaseWebDavPath() {
    return String(
      state.data.meta?.studentDatabaseWebDavPath || DEFAULT_STUDENT_DATABASE_WEBDAV_PATH
    ).trim();
  }

  function getYandexDiskBasePath() {
    return String(
      state.data.meta?.yandexDiskBasePath || DEFAULT_YANDEX_DISK_BASE_PATH
    ).trim();
  }
  ...
  async function saveYandexDiskSettings(form) {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(payload.error || "Не удалось сохранить настройки Яндекс-Диска.");
    if (form.elements.yandexDiskPassword) form.elements.yandexDiskPassword.value = "";
  ...
  function applyYandexDiskSettings(payload = {}) {
    state.data.meta.studentDatabaseWebDavPath = String(
    ...
    state.data.meta.studentApplicationsEmailHost = String(
      payload.emailHost ?? getStudentApplicationsEmailHost()
    ).trim();
  ...
  async function testYandexDiskConnection(event) {
    ...
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || "Не удалось подключиться к WebDAV.");
      alert(payload.message || "Подключение WebDAV работает.");
      render();
    } catch (error) {
      alert(`Проверка WebDAV не выполнена: ${error.message}`);
      button.disabled = false;
    }
  }

  async function testStudentApplicationsEmailConnection(event) {
    const button = event.currentTarget;
    const form = button.closest("form");
    if (!form) return;
    button.disabled = true;
  ...
  function waitForStudentImportPoll(delay = 200) {
    return new Promise((resolve) => window.setTimeout(resolve, delay));
  }

  function formatDatabaseOperationDuration(startedAt) {
  ...
  async function runStudentDatabaseImport(databasePath) {
    ...
    while (true) {
      ...
      updateDatabaseImportIndicator({
        status: status.message || "Обработка базы...",
        progress: Number(status.progress) || 0,
        indeterminate: false
      });
      if (status.status === "failed") {
        throw new Error(status.error || status.message || "Импорт завершился с ошибкой.");
      }
  ...
  function buildStudentDatabaseExportDirectExpenses() {
    const students = state.data.collections.students || [];
    const linkedExpenses = students.flatMap((student) => (
      (Array.isArray(student.directExpenses) ? student.directExpenses : []).map((expense) => ({
        ...expense,
        uid: String(expense.uid || student.uid || "").trim()
      }))
    ));
    const unlinkedExpenses = (state.data.collections.directExpenses || []).map((expense) => ({ ...expense }));
    const manualExpenses = students.flatMap((student) => (
      Array.from({ length: 6 }, (_, index) => {
        const number = index + 1;
        const date = String(student[`expense${number}Date`] || "").trim();
        const type = String(student[`expense${number}Type`] || "").trim();
        const note = String(student[`expense${number}Note`] || "").trim();
        const amountValue = student[`expense${number}Amount`];
        const hasAmount = amountValue !== "" && amountValue !== null && amountValue !== undefined;
        if (!date && !type && !note && !hasAmount) return null;
        return {
          uid: String(student.uid || "").trim(),
          date,
          type,
          amount: hasAmount ? Number(amountValue) || 0 : "",
          note,
          inventoryLink: String(student[`expense${number}InventoryLink`] || "").trim()
        };
      }).filter(Boolean)
    ));
  ...
  async function importStudentsFromDatabase(event) {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Не удалось сохранить загруженные данные: ${error.message}`);
      }
      render();
  ...
  function sumStudentExpenses(record) {
    const manualExpenses = Array.from({ length: 6 }, (_, index) => Number(record[`expense${index + 1}Amount`] || 0))
      .reduce((sum, value) => sum + value, 0);
    const linkedExpenses = (Array.isArray(record.directExpenses) ? record.directExpenses : [])
      .reduce((sum, expense) => sum + Number(expense?.amount || 0), 0);
    return manualExpenses + linkedExpenses;
  }
  ...
  function buildFinanceSeries() {
    ...
    getAllDirectExpenses().forEach((expense) => {
  ...
  function getStudentCompactFolderName(value) {
    const parts = String(value || "").normalize("NFC").trim().split(/\s+/).filter(Boolean);
    if (!parts.length) return "";
    const surname = parts[0].replace(/[\\/:*?"<>|]+/g, "").replace(/[^\p{L}\p{N}().-]+/gu, "");
    const initials = [];
    for (const part of parts.slice(1)) {
      const letters = part.match(/\p{L}/gu) || [];
      if (!letters.length) continue;
      if (part.includes(".")) initials.push(...letters);
      else initials.push(letters[0]);
      if (initials.length >= 2) break;
    }
    return `${surname}${initials.slice(0, 2).join("")}`
      .replace(/[^\p{L}\p{N}().-]+/gu, "")
      .slice(0, 80);
  }
  ...
  function normalizeStudentDocumentsFolderSource(value, stripFileName = false) {
    let source = String(value || "")
      ...
      .replace(/\\/g, "/")
      .replace(/^[a-z]:\/+/i, "");
    const useParentFolder = /^\[-1\](?:\/+|$)/u.test(source);
    source = source.replace(/^\[-1\]\/+/u, "");
  ...
  function getStudentYandexDocumentsFolderUrl(record) {
    ...
    const baseParts = getYandexDiskBasePath()
      ...
      .filter(Boolean);
    if (useParentFolder) baseParts.pop();
    const fullPath = [...baseParts, ...folder
      .replace(/\\/g, "/")
      .split("/")
      .map((part) => part.trim())
      .filter(Boolean)];
  ...
  function readYandexDocumentSaveResult(response, requested) {
    ...
    if (saved === "true") {
      return { saved: true, path: decodeHeader("X-Yandex-Disk-Path") };
    }
    return {
      saved: false,
      error: decodeHeader("X-Yandex-Disk-Error") || "Яндекс-Диск не подтвердил сохранение файла."
    };
  }

  function showYandexDocumentSaveWarning(result) {
  ...
  async function openStudentPostalEnvelopeDocument(event) {
  ...
  function ensureStudentOrderDocumentDate(record, orderFieldName, fieldName, currentMessage, groupMessagePrefix) {
    const records = getStudentOrderDocumentRecords(record, orderFieldName);
    const checkedRecords = records.length ? records : [record];
    const missingRecord = checkedRecords.find((item) => !parseOrdersSdoDate(item?.[fieldName]));
    if (!missingRecord) return true;
    const currentId = String(record?.id || state.modal?.id || "").trim();
    const missingId = String(missingRecord?.id || "").trim();
    const isCurrentRecord = !missingId || (currentId && missingId === currentId);
    if (isCurrentRecord) {
      alert(currentMessage);
      document.querySelector(`[name='${fieldName}']`)?.focus({ preventScroll: true });
    } else {
      const name = String(missingRecord.name || "").trim() || "слушатель без ФИО";
      alert(`${groupMessagePrefix}: ${name}.`);
    }
    return false;
  }
  ...
  async function openStudentEnrollmentOrderDocument(event) {
    ...
    if (!ensureStudentOrderDocumentDate(
      ...
      "enrollmentOrderNo",
      "enrollmentDate",
      "Заполните дату зачисления перед формированием приказа на зачисление.",
      "В приказ на зачисление попадает слушатель без даты зачисления"
    )) return;
    if (!ensureStudentOrderDocumentDate(
      record,
      "enrollmentOrderNo",
      "startDate",
      "Заполните дату начала обучения перед формированием приказа на зачисление.",
  ...
  async function openStudentExpulsionOrderDocument(event) {
    ...
    if (!ensureStudentOrderDocumentDate(
      ...
      "expulsionOrderNo",
      "expulsionDate",
      "Заполните дату отчисления перед формированием приказа об отчислении.",
  ...
  function formatOrderDocumentNumberedList(records, formatItem) {
    return records
      .map(formatItem)
      .filter(Boolean)
      .join("\n");
  ...
  function formatEnrollmentOrderStudentList(record) {
    return formatOrderDocumentNumberedList(
      getStudentOrderDocumentRecords(record, "enrollmentOrderNo"),
      formatEnrollmentOrderStudentListItem
    );
  }
  ...
  function formatExpulsionOrderStudentList(record, issuedFilter = null) {
    return formatOrderDocumentNumberedList(
      getExpulsionOrderDocumentRecords(record, issuedFilter),
      formatExpulsionOrderStudentListItem
    );
  }
  ...
  function evaluateContractTemplateField(field, record, values, evaluateByName = null) {
    ...
    if (fieldName === "N2") {
      ...
      if (!withoutIssued) return "";
      return withIssued ? "2" : "1";
    }
    if (fieldName === "N3") {
      const withIssued = getExpulsionOrderDocumentRecords(record, true).length;
      const withoutIssued = getExpulsionOrderDocumentRecords(record, false).length;
  ...
  function getContractTemplateRawSourceValue(name, record) {
    ...
    if ([
      ...
      return String(record?.expulsionDate || record?.expulsionOrderDate || "").trim();
    }
    if (normalized === "Пол") {
    ...
    if (normalized === "Стажировка") return isChecked(record.internship) ? "+" : "";
  ...
  function copyStudentPassportToCustomer() {
    ...
    fieldPairs.forEach(([sourceKey, targetKey]) => {
      ...
      if (!target.matches("[data-combo-input]")) {
        target.dispatchEvent(new Event("input", { bubbles: true }));
      }
      target.dispatchEvent(new Event("change", { bubbles: true }));
      copied += 1;
